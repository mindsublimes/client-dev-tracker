class DashboardsController < ApplicationController
  def show
    ...
    @client_snapshots = client_scope.map do |client|
      client_items = scope.where(client_id: client.id)
      open_items = client_items.pending
      recent_velocity = client_items.where(status: :completed).where('updated_at > ?', 14.days.ago).count
      {
        client:,
